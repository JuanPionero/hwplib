...
import com.agadasom.hwplib.object.bodytext.paragraph.text.HWPCharNormal;
...
public class ParaTextSetter {
    ...
    public static void deleteParaTextTo(Paragraph p, int to) {
        ...
        if (p.getText() != null) {
            for (int charIndex = 0; charIndex < to + 1
                    && charIndex < p.getText().getCharList().size() - 1; charIndex++) {
                HWPChar hwpChar = p.getText().getCharList().get(charIndex);
                if (hwpChar.getType() == HWPCharType.ControlExtend) {
                    deleteCtrlCount++;
                }
