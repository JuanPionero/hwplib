package com.agadasom.hwplib.tool.paragraphadder;

import com.agadasom.hwplib.object.bodytext.control.*;
...
import com.agadasom.hwplib.object.bodytext.paragraph.charshape.CharPositionShapeIdPair;
...
import com.agadasom.hwplib.tool.paragraphadder.control.*;
...
public class ParagraphCopier {
    ...
    private void copyHeader() {
        if (source.getHeader() != null) {
            ...
            targetH.setParaShapeId((docInfoAdder == null) ? sourceH.getParaShapeId()
                    : docInfoAdder.forParaShape().processById(sourceH.getParaShapeId()));
            targetH.setStyleId((docInfoAdder == null) ? sourceH.getStyleId()
                    : (short) docInfoAdder.forStyle().processById(sourceH.getStyleId()));
        ...
        if (source.getCharShape() != null) {
            ...
            for (CharPositionShapeIdPair cpsp : source.getCharShape().getPositonShapeIdPairList()) {
                if (excludedSectionDefine == true && cpsp.getPosition() > 0) {
                    target.getCharShape().addParaCharShape(cpsp.getPosition() - 8,
                            (docInfoAdder == null) ? cpsp.getShapeId()
                                    : docInfoAdder.forCharShape().processById((int) cpsp.getShapeId()));
                } else {
                    target.getCharShape().addParaCharShape(cpsp.getPosition(),
                            (docInfoAdder == null) ? cpsp.getShapeId()
                                    : docInfoAdder.forCharShape().processById((int) cpsp.getShapeId()));
                }
            }
    ...
    private void copyControlList() {
        ...
        for (Control c : source.getControlList()) {
            switch (c.getType()) {
                case Table:
                    TableCopier.copy((ControlTable) c, (ControlTable) target.addNewControl(ControlType.Table),
                            docInfoAdder);
                    break;
                case Gso:
                    GsoCopier.copy((GsoControl) c, (GsoControl) target.addNewGsoControl(((GsoControl) c).getGsoType()),
                            docInfoAdder);
                    break;
                case Equation:
                    EquationCopier.copy((ControlEquation) c,
                            (ControlEquation) target.addNewControl(ControlType.Equation), docInfoAdder);
                    break;
                case SectionDefine:
                    if (includingSectionInfo) {
                        SectionDefineCopier.copy((ControlSectionDefine) c,
                                (ControlSectionDefine) target.addNewControl(ControlType.SectionDefine), docInfoAdder);
                    }
                    break;
                case ColumnDefine:
                    ETCControlCopier.copyColumnDefine((ControlColumnDefine) c,
                            (ControlColumnDefine) target.addNewControl(ControlType.ColumnDefine), docInfoAdder);
                    break;
                case Header:
                    if (includingSectionInfo) {
                        ETCControlCopier.copyHeader((ControlHeader) c,
                                (ControlHeader) target.addNewControl(ControlType.Header), docInfoAdder);
                    }
                    break;
                case Footer:
                    if (includingSectionInfo) {
                        ETCControlCopier.copyFooter((ControlFooter) c,
                                (ControlFooter) target.addNewControl(ControlType.Footer), docInfoAdder);
                    }
                    break;
                case Footnote:
                    ETCControlCopier.copyFootnote((ControlFootnote) c,
                            (ControlFootnote) target.addNewControl(ControlType.Footnote), docInfoAdder);
                    break;
                case Endnote:
                    ETCControlCopier.copyEndnote((ControlEndnote) c,
                            (ControlEndnote) target.addNewControl(ControlType.Endnote), docInfoAdder);
                    break;
                case AutoNumber:
                    ETCControlCopier.copyAutoNumber((ControlAutoNumber) c,
                            (ControlAutoNumber) target.addNewControl(ControlType.AutoNumber), docInfoAdder);
                    break;
                case NewNumber:
                    ETCControlCopier.copyNewNumber((ControlNewNumber) c,
                            (ControlNewNumber) target.addNewControl(ControlType.NewNumber), docInfoAdder);
                    break;
                case PageHide:
                    ETCControlCopier.copyPageHide((ControlPageHide) c,
                            (ControlPageHide) target.addNewControl(ControlType.PageHide), docInfoAdder);
                    break;
                case PageOddEvenAdjust:
                    ETCControlCopier.copyPageOddEvenAdjust((ControlPageOddEvenAdjust) c,
                            (ControlPageOddEvenAdjust) target.addNewControl(ControlType.PageOddEvenAdjust),
                            docInfoAdder);
                    break;
                case PageNumberPosition:
                    ETCControlCopier.copyPageNumberPosition((ControlPageNumberPosition) c,
                            (ControlPageNumberPosition) target.addNewControl(ControlType.PageNumberPosition),
                            docInfoAdder);
                    break;
                case IndexMark:
                    ETCControlCopier.copyIndexMark((ControlIndexMark) c,
                            (ControlIndexMark) target.addNewControl(ControlType.IndexMark), docInfoAdder);
                    break;
                case Bookmark:
                    ETCControlCopier.copyBookmark((ControlBookmark) c,
                            (ControlBookmark) target.addNewControl(ControlType.Bookmark), docInfoAdder);
                    break;
                case OverlappingLetter:
                    OverlappingLetterCopier.copy((ControlOverlappingLetter) c,
                            (ControlOverlappingLetter) target.addNewControl(ControlType.OverlappingLetter),
                            docInfoAdder); // 신규 추가
                    break;
                case AdditionalText:
                    AdditionalTextCopier.copy((ControlAdditionalText) c,
                            (ControlAdditionalText) target.addNewControl(ControlType.AdditionalText), docInfoAdder);
                    break;
                case HiddenComment:
                    ETCControlCopier.copyHiddenComment((ControlHiddenComment) c,
                            (ControlHiddenComment) target.addNewControl(ControlType.HiddenComment), docInfoAdder);
                    break;
                case Form:
                    ETCControlCopier.copyForm((ControlForm) c, (ControlForm) target.addNewControl(ControlType.Form),
                            docInfoAdder);
                    break;
                case FIELD_UNKNOWN:
                case FIELD_DATE:
                ...
                case FIELD_TABLEOFCONTENTS:
                    ETCControlCopier.copyField((ControlField) c,
                            (ControlField) target.addNewControl(((ControlField) c).getHeader().getCtrlId()),
                            docInfoAdder);
                    break;
            }
        }
    ...
    public static void copyMemoList(Paragraph source, Paragraph target, DocInfoAdder docInfoAdder) {
         ...
         * ParagraphCopier.listCopy(memo.getParagraphList(), cloned.getParagraphList(),
         * docInfoAdder);
         * }
         * 
         */
