...
import com.agadasom.hwplib.object.docinfo.borderfill.fillinfo.*;
...
public class GsoCommonPartCopier {
    public static void copy(GsoControl source, GsoControl target, DocInfoAdder docInfoAdder) {
        ...
        if (source.getGsoType() == GsoControlType.Container) {
            shapeComponentGroup((ShapeComponentContainer) source.getShapeComponent(),
                    (ShapeComponentContainer) target.getShapeComponent());
        } else {
            shapeComponentNormal((ShapeComponentNormal) source.getShapeComponent(),
                    (ShapeComponentNormal) target.getShapeComponent(), docInfoAdder);
        }
    }
    ...
    private static void shapeComponentNormal(ShapeComponentNormal source, ShapeComponentNormal target,
    ...
    public static void pictureInfo(PictureInfo source, PictureInfo target, DocInfoAdder docInfoAdder) {
        ...
        target.setBinItemID((docInfoAdder == null) ? source.getBinItemID()
                : docInfoAdder.forBinData().processById(source.getBinItemID()));
    }
}
