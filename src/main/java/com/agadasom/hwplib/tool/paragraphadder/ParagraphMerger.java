...
import com.agadasom.hwplib.object.bodytext.paragraph.text.HWPChar;
import com.agadasom.hwplib.object.bodytext.paragraph.text.HWPCharControlExtend;
import com.agadasom.hwplib.object.bodytext.paragraph.text.HWPCharType;
...
public class ParagraphMerger {
    ...
    private void moveCharSpace() {
        if (sourceCharShapeIndex < source.getCharShape().getPositonShapeIdPairList().size()) {
            ...
            if (cpsip.getPosition() <= sourceCharPosition) {
                target.getCharShape().addParaCharShape(targetCharPosition, (docInfoAdder == null) ? cpsip.getShapeId()
                        : docInfoAdder.forCharShape().processById((int) cpsip.getShapeId()));
    ...
    private void moveControl(Control sourceControl) {
        ...
        switch (sourceControl.getType()) {
            case Table:
                TableCopier.copy((ControlTable) sourceControl, (ControlTable) target.addNewControl(ControlType.Table),
                        docInfoAdder);
                break;
            case Gso:
                GsoCopier.copy((GsoControl) sourceControl,
                        target.addNewGsoControl(((GsoControl) sourceControl).getGsoType()), docInfoAdder);
                break;
            case Equation:
                EquationCopier.copy((ControlEquation) sourceControl,
                        (ControlEquation) target.addNewControl(ControlType.Equation), docInfoAdder);
                break;
            case Header:
                ETCControlCopier.copyHeader((ControlHeader) sourceControl,
                        (ControlHeader) target.addNewControl(ControlType.Header), docInfoAdder);
                break;
            case Footer:
                ETCControlCopier.copyFooter((ControlFooter) sourceControl,
                        (ControlFooter) target.addNewControl(ControlType.Footer), docInfoAdder);
                break;
            case Footnote:
                ETCControlCopier.copyFootnote((ControlFootnote) sourceControl,
                        (ControlFootnote) target.addNewControl(ControlType.Footnote), docInfoAdder);
                break;
            case Endnote:
                ETCControlCopier.copyEndnote((ControlEndnote) sourceControl,
                        (ControlEndnote) target.addNewControl(ControlType.Endnote), docInfoAdder);
                break;
            case AutoNumber:
                ETCControlCopier.copyAutoNumber((ControlAutoNumber) sourceControl,
                        (ControlAutoNumber) target.addNewControl(ControlType.AutoNumber), docInfoAdder);
                break;
            case NewNumber:
                ETCControlCopier.copyNewNumber((ControlNewNumber) sourceControl,
                        (ControlNewNumber) target.addNewControl(ControlType.NewNumber), docInfoAdder);
                break;
            case PageHide:
                ETCControlCopier.copyPageHide((ControlPageHide) sourceControl,
                        (ControlPageHide) target.addNewControl(ControlType.PageHide), docInfoAdder);
                break;
            case PageOddEvenAdjust:
                ETCControlCopier.copyPageOddEvenAdjust((ControlPageOddEvenAdjust) sourceControl,
                        (ControlPageOddEvenAdjust) target.addNewControl(ControlType.PageOddEvenAdjust), docInfoAdder);
                break;
            case PageNumberPosition:
                ETCControlCopier.copyPageNumberPosition((ControlPageNumberPosition) sourceControl,
                        (ControlPageNumberPosition) target.addNewControl(ControlType.PageNumberPosition), docInfoAdder);
                break;
            case IndexMark:
                ETCControlCopier.copyIndexMark((ControlIndexMark) sourceControl,
                        (ControlIndexMark) target.addNewControl(ControlType.IndexMark), docInfoAdder);
                break;
            case Bookmark:
                ETCControlCopier.copyBookmark((ControlBookmark) sourceControl,
                        (ControlBookmark) target.addNewControl(ControlType.Bookmark), docInfoAdder);
                break;
            case OverlappingLetter:
                OverlappingLetterCopier.copy((ControlOverlappingLetter) sourceControl,
                        (ControlOverlappingLetter) target.addNewControl(ControlType.OverlappingLetter), docInfoAdder); // 신규
                                                                                                                       // 추가
                break;
            case AdditionalText:
                AdditionalTextCopier.copy((ControlAdditionalText) sourceControl,
                        (ControlAdditionalText) target.addNewControl(ControlType.AdditionalText), docInfoAdder);
                break;
            case HiddenComment:
                ETCControlCopier.copyHiddenComment((ControlHiddenComment) sourceControl,
                        (ControlHiddenComment) target.addNewControl(ControlType.HiddenComment), docInfoAdder);
                break;
            case Form:
                ETCControlCopier.copyForm((ControlForm) sourceControl,
                        (ControlForm) target.addNewControl(ControlType.Form), docInfoAdder);
                break;
            case FIELD_UNKNOWN:
            case FIELD_DATE:
            case FIELD_DOCDATE:
            ...
            case FIELD_TABLEOFCONTENTS:
                ETCControlCopier.copyField((ControlField) sourceControl,
                        (ControlField) target.addNewControl(((ControlField) sourceControl).getHeader().getCtrlId()),
                        docInfoAdder);
                break;
        }
    }
