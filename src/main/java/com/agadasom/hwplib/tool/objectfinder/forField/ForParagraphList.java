package com.agadasom.hwplib.tool.objectfinder.forField;
...
import com.agadasom.hwplib.tool.objectfinder.TextBuffer;
import com.agadasom.hwplib.tool.objectfinder.forField.gettext.ForControl;
import com.agadasom.hwplib.tool.objectfinder.forField.gettext.ForControlWithAllField;
import com.agadasom.hwplib.tool.paragraphadder.ParaTextSetter;
...
public class ForParagraphList {
     ...
     * @throws UnsupportedEncodingException
     */
    private static ArrayList<FindPosition> getFieldStartPosition(ParagraphListInterface paragraphList,
            ControlType fieldType, String fieldName, boolean allField) {
     ...
     */
    private static FindPosition getFieldStartPosition(ParagraphListInterface paragraphList, ControlType fieldType,
            String fieldName, int startParaIndex, int startCharIndex) {
        int paraCount = paragraphList.getParagraphCount();
     ...
     * @throws UnsupportedEncodingException
     */
    private static String getText(ParagraphListInterface paragraphList, FindPosition position,
            TextExtractMethod temInField) throws UnsupportedEncodingException {
        ...
        for (int paraIndex = position.startParaIndex; paraIndex <= position.endParaIndex; paraIndex++) {
            ...
            int endCharIndex = (paraIndex == position.endParaIndex) ? position.endCharIndex
                    : (paragraphList.getParagraph(paraIndex).getText() == null) ? 0
                            : paragraphList.getParagraph(paraIndex).getText().getCharList().size();
            com.agadasom.hwplib.tool.textextractor.ForParagraph.extract(paragraphList.getParagraph(paraIndex),
     ...
     */
    public static void getAllFieldText(ParagraphListInterface paragraphList, ControlType fieldType, String fieldName,
     ...
     * @param textList      반환할 필드 텍스트 리스트
     */
    private static void getAllFieldTextForControl(ParagraphListInterface paragraphList, ControlType fieldType,
            String fieldName, TextExtractMethod temInField, ArrayList<String> textList)
            throws UnsupportedEncodingException {
        for (Paragraph p : paragraphList) {
            ArrayList<Control> controlList = p.getControlList();
            if (controlList != null) {
    ...
    public static SetFieldResult setFieldText(ParagraphListInterface paragraphList, ControlType fieldType,
        ...
        while (result != null) {
            ...
            result = getFieldStartPosition(paragraphList, fieldType, fieldName, result.endParaIndex,
                    result.endCharIndex + 1);
        }
     ...
     * @param text          텍스트
     */
    private static void changeText(ParagraphListInterface paragraphList, FindPosition position, String text)
            throws UnsupportedEncodingException {
        if (position.startParaIndex != position.endParaIndex) {
            ...
            if (position.endParaIndex - position.startParaIndex >= 2) {
                for (int deleteIndex = 0; deleteIndex < position.endParaIndex - position.startParaIndex
                        - 1; deleteIndex++) {
     ...
     * @return 필드 설정 결과값
     */
    private static SetFieldResult setFieldTextForControls(ParagraphListInterface paragraphList, ControlType fieldType,
            String fieldName,
        ...
        for (Paragraph p : paragraphList) {
            ...
            if (controlList != null) {
                for (Control c : controlList) {
                    if (com.agadasom.hwplib.tool.objectfinder.forField.settext.ForControl.setFieldText(c, fieldType,
